...
function styleChildrenBorder(): React.CSSProperties { return { borderLeft: "1px solid #374151" }; }
...
const NodeBox: React.FC<{
  ...
  return (
    <div className={`flex flex-col sm:flex-row items-start gap-1.5 sm:gap-3 my-0.5`}>
      ...
      {open && hasChildren && (
        <span className="sm:hidden inline-block" style={{ width: 16, height: 10, borderLeft: "1px solid #4b5563", borderBottom: "1px solid #4b5563", marginLeft: "1rem", borderBottomLeftRadius: 8 }} />
      )}
      ...
      {open && hasChildren && (
        <div className="pl-3 sm:pl-4 flex flex-col gap-1.5 sm:gap-2 w-full" style={styleChildrenBorder()}>
          {children.map((c, i) => (
            <NodeBox
              key={c.id} node={c} level={level + 1} idx={i}
...
const MindMapView: React.FC<Props> = ({ data, summaryTitle, onBack }) => {
  ...
  // === Descargar HTML con el mismo look & feel
  const downloadHTML = () => {
    const isContentfulLocal = (n?: Partial<MindMapNode>) => Boolean(String(n?.label ?? "").trim() || String(n?.note ?? "").trim());
